...
(aio-defun leetcode-try ()
  ...
  (let* ((code-buf (current-buffer))
    ...
    (leetcode--debug "slug title: %s" slug-title)
...
(defvar leetcode-save-solution nil
...
(defun leetcode--get-code-buffer (title problem-id)
  (if leetcode-save-solution
      (find-file-noselect
       (format "%s/%04d_%s"
               leetcode-directory
               problem-id
               (leetcode--get-code-buffer-name title)))
    (get-buffer-create (leetcode--get-code-buffer-name title))))
...
for current problem."
  ...
  (let* ((code-buf (get-buffer (leetcode--get-code-buffer-name title)))
    ...
    (unless code-buf
      (with-current-buffer (leetcode--get-code-buffer title problem-id)
